...
public class ActivityTeach extends BaseActivity {
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView((int) R.layout.activity_teach);
        StringBuilder sb = new StringBuilder();
        sb.append(String.format("屏幕大小：(%d, %d)\n", new Object[]{SysUtils.screenWidth(this.context), Integer.valueOf(SysUtils.screenHeight(this.context))}));
        sb.append("screenWidth(dp)：").append(SysUtils.px2dp((float) SysUtils.screenWidth(this.context))).append(CommandExecution.COMMAND_LINE_END);
        ...
        sb.append(String.format("内存：总共：%s，剩余：%s\n", CommonUtils.formatSize((double) memoryInfo.totalMem, 1000), CommonUtils.formatSize((double) memoryInfo.availMem, 1000)));
        sb.append(CommandExecution.COMMAND_LINE_END);
